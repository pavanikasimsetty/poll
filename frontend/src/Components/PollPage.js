...
export default function PollPage() {
    ...
    useEffect(() => {
        const fetchPollData = async () => {
            try {
                const pollResponse = await fetch(`https://poll-red.vercel.app/api/polls/${pollId}`);
                if (!pollResponse.ok) {
            ...
            try {
                ...
                for (const option in formattedData) {
                    ...
                    const userNames = await Promise.all(userIds.map(async userId => {
                        const userResponse = await fetch(`https://poll-red.vercel.app/api/studentauth/getusername/${userId}`);
                        if (userResponse.ok) {
