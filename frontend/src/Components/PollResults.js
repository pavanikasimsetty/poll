...
const PollResults = () => {
  ...
  const fetchPollHistory = async () => {
    try {
      const response = await fetch(`https://poll-red.vercel.app/api/polls/history/active/${userId}`);
      if (response.ok) {
  ...
  const endPolling = async (pollId) => {
    try {
      const response = await fetch(`https://poll-red.vercel.app/api/polls/end/${pollId}`, {
        method: 'PUT',
        headers: {
