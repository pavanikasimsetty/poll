...
const Singlepoll = () => {
  ...
  useEffect(() => {
    const fetchPoll = async () => {
      try {
        const pollResponse = await fetch(`https://poll-red.vercel.app/api/pollResponses/fetchpoll/${userId}/${pollId}`);
  ...
  const checkHasVoted = async (pollId) => {
    try {
      const response = await fetch(`https://poll-red.vercel.app/api/pollResponses/hasVoted/${userId}/${pollId}`);
      if (response.ok) {
