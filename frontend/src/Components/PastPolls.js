...
const socket = io('https://poll-red.vercel.app/');
...
const StudentDashboard = () => {
  ...
  useEffect(() => {
    const fetchPollsAndResponses = async () => {
      try {
        const userId = localStorage.getItem('userId');
        const response = await fetch('https://poll-red.vercel.app/api/polls/deactive');
        if (response.ok) {
          const data = await response.json();
          const fetchedPolls = data.polls;
  
          const responsePromises = fetchedPolls.map(async (poll) => {
            const pollResponse = await fetch(`https://poll-red.vercel.app/api/pollResponses/fetchpoll/${userId}/${poll._id}`);
            if (pollResponse.ok) {
  ...
  const submitPollResponse = async (pollId, selectedOption) => {
    try {
      ...
      const response = await fetch('https://poll-red.vercel.app/api/pollResponses', {
        method: 'POST',
        headers: {
  ...
  const checkHasVoted = async () => {
    try {
      ...
      const promises = polls.map(async (poll) => {
        const response = await fetch(`https://poll-red.vercel.app/api/pollResponses/hasVoted/${userId}/${poll._id}`);
        if (response.ok) {
          const data = await response.json();
