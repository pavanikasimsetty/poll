...
const PollHistory = () => {
  ...
  const fetchPollHistory = async () => {
    try {
      const response = await fetch(`https://poll-red.vercel.app/api/polls/history/deactive/${userId}`);
      if (response.ok) {
  ...
  const reopenPolling = async (pollId) => {
    try {
      const response = await fetch(`https://poll-red.vercel.app/api/polls/reopen/${pollId}`, {
        method: 'PUT',
        headers: {
